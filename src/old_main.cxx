...
void writeNetwork(std::string filename,
...
std::vector<std::vector<std::vector<double>>> readNetwork(
    std::string filename, std::vector<int> nodes) {
  std::vector<std::vector<std::vector<double>>> network;
...
std::vector<std::vector<std::vector<double>>> createNetwork(
...
double sigmoid(double z) { return 1 / (1 + expf(-z)); }

std::vector<std::vector<double>> getActivations(
    std::vector<std::vector<std::vector<double>>> network,
...
std::vector<std::vector<double>> getDelta(
    std::vector<std::vector<std::vector<double>>> network,
    std::vector<std::vector<double>> activations, std::vector<double> desired) {
  std::vector<std::vector<double>> delta = activations;

  for (int i = 0; i < activations.back().size(); i++) {
    // Find regularization error
    delta[delta.size() - 1][i] = 2 * (activations.back()[i] - desired[i]) *
                                 activations.back()[i] *
                                 (1 - activations.back()[i]);
  }
...
void test(std::vector<std::vector<std::vector<double>>> network) {
  ...
  std::cout << "Correct: " << correct << ", Total: " << test_length
            << ", Accuracy: " << (double)correct / test_length << '\n';
}

void backpropigation(std::vector<std::vector<std::vector<double>>>& network,
                     int epochs) {
  ...
  for (int batch = 0; batch < epochs * DATASET_SIZE / BATCH_SIZE; batch++) {
    ...
    for (int sample = 0; sample < BATCH_SIZE; sample++) {
      ...
      // for (int i = 0; i < delta[0].size(); i++) std::cout << (delta[0][i] >
      // 0.0f) << ' ';
      /*
      for (auto a : delta) {
        for (auto b : a) {
    ...
    for (int i = 0; i < network.size(); i++) {
      for (int j = 0; j < network[i].size(); j++) {
        for (int k = 0; k < network[i][j].size() - 1; k++) {
          network[i][j][k] =
              network[i][j][k] *
                  (1 - LEARNING_RATE * REGULARIZATION / BATCH_SIZE) -
              LEARNING_RATE * slope[i][j][k];
    ...
    if ((batch * BATCH_SIZE) % 10000 == 0 && batch != 0)
...
void custom_network() {
  ...
  */
  ...
  for (int i = 0; i < 1; i++) {
    ...
    // std::cout << delta[1][0] << " ";
    // std::cout << (desired[0] - activations.back()[0]) * (desired[0] -
    // activations.back()[0]) << ", "; std::cout << network[1][0][0] << ' ';
